...
export default class SpeakerPageEU extends React.Component {
  handleClick(e) {
    if(window.fromHome){
  ...
  render() {
    ...
    let sessions = data.findSessionBySpeakerSlug(this.props.params.slug, 'eu2017')
    let sessionSection
    ...
    let workshops = data.findWorkshopBySpeakerSlug(this.props.params.slug, 'eu2017')
    let workshopSection
    ...
    return (
      <div id="content" className="speaker-page-content">
        <header className="speaker-page--header">
          <Link to="/eu2017/" onClick={this.handleClick.bind(this)}>
            <img src="/img/pgd_logo_eu-2017.svg" alt="PhoneGap Day EU logo"/>
          </Link>
        </header>
        <div className="text-bounds speaker">
          <Link to="/eu2017/" onClick={this.handleClick.bind(this)}>&lt; {this.fromHome ? "Back" : "Home"}</Link>
